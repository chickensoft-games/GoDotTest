...
- `--run-tests`: The presence of this flag informs your game that tests should be run. If you've setup your main scene to redirect to the test scene when it finds this flag (as described above), you can use pass this flag in when running Godot from the command line (for debugging or CI/CD purposes) to run your test(s). You can pass a specific test suite name to run just that test suite via `--run-tests=TestSuiteName` or even a single test method in a test suite with `--run-tests=TestSuiteName.TestMethodName`. If you do not pass a test suite name, GoDotTest will run all test suites in the current assembly.
